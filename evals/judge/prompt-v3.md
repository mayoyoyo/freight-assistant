<!--
<!-- v3 = v2 with rules 6/7 example phrases rewritten HELD-OUT. v2's examples
     quoted CAL05/CAL06 drafts verbatim, contaminating its calibration score
     (Codex review of PR #4, finding 5). Rule semantics unchanged. -->
Judge prompt v2 — 2026-08-06 — model claude-opus-5, extended thinking disabled.

CHANGES FROM v1 (v1 scored no_invented_commitments TPR 76.9% / TNR 100%,
professional_tone TPR 100% / TNR 100%):
  1. Added the frozen REFERENCE_DATE to the groundedness check's domain section.
     v1 had no notion of "today", so it read "pickup is today" against the
     inquiry's occurred_at and called a correct relative date an invented
     schedule detail.
  2. Scoped the check explicitly to claims about the freight and to terms
     offered to the carrier, and added two does-NOT-count carve-outs:
     (a) characterizations of the carrier's own business that carry no number
     or term, (b) statements of Goodlane's own commercial disposition
     (covered / passing / going another direction).
     v1 failed drafts for both — neither is an invented commitment or
     hallucinated logistics, which is what this check is named for.
  The professional_tone section is BYTE-IDENTICAL to v1 on purpose: it scored
  100/100 and there was nothing to diagnose, so any v2 movement there is judge
...
fact, figure, logistics detail, or commercial commitment **about the freight or
about the terms offered to this carrier** that the source records do not
support.

**Today's date is 2026-05-25.** The corpus is a frozen snapshot ending that day
and every draft is written on that day. Resolve "today", "tomorrow", "this
week" and weekday names against 2026-05-25, never against a record's
`occurred_at` and never against your own sense of the date. A load whose
`pickup_date` is 2026-05-25 is correctly described as picking up today.
...
Read every assertion the draft makes **about the freight (rate, schedule, lane,
weight, equipment, facility, handling) or about the terms Goodlane is offering
this carrier**. Those are the assertions in scope. Output **FAIL** if ANY ONE of
these holds:
...
When you are genuinely torn on a borderline phrase, ask two questions in order:
*(1) Is this a claim about the freight or about the terms we are offering?* If
no — it is courtesy, relationship framing, or a decision Goodlane is announcing
about its own book — PASS. *(2) If yes, could the carrier enforce it against
Goodlane or dispatch a truck on it, and is there a record backing it?* No record
plus enforceable or operationally actionable ⇒ FAIL.
...
6. **Characterizations of the carrier's own business.** "Since you usually run
   the I-81 corridor", "this one's close to your yard", "I know your drivers
   prefer weekend pickups". These are relationship framing addressed to the party who
   is the actual authority on the subject; they bind Goodlane to nothing and
   the carrier can correct them in one line. Even a loose or partly-off
   characterization is not an invented commitment. Fail only if the framing
   carries a specific number or term (e.g. "your standing $2.10/mile on this
   lane") that no record supports.
7. **Goodlane's own commercial disposition.** "That one's already spoken for",
   "the truck is booked", "we're holding off for now", "we'll sit this round
   out". These are statements about a decision the broker owns, not
   claims about the freight's specifications and not commitments offered to the
   carrier. The load table's `status` field is a lagging snapshot, so a
   `status: "open"` row does NOT by itself make "we covered it" an invented
   fact. Do not fail a draft for declining or withdrawing a load.
