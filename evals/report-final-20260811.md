...
| field | value |
| --- | --- |
| run id | opus-final2-20260811 |
| label | **FINAL (2026-08-11)** — post-adjudication: tool fixes (fused lane token, name-keyed carrier lookup), sp-v3+set-contract prompt, dated gold/spec corrections |
| model | claude-opus-5 |
| prompt version | sp-v3 |
| judge version | v3 |
| cases | 24 (`cases.jsonl`) |
| runs | 72 (k=3) |
| runs file | `evals/results/runs-opus-final2-20260811.jsonl` |
...
| --- | --- | --- | --- |
| **Run-level pass rate** | 66/72 (91.7%) | [83.0%, 96.1%] | every graded run, all k pooled — the expected outcome of one broker query |
| **pass@1** | 22/24 (91.7%) | [74.2%, 97.7%] | case passes if its first run passes (the k=1 slice) |
| **pass^3** | 21/24 (87.5%) | [69.0%, 95.7%] | case passes only if ALL k runs pass — our own addition, see note |
...
| --- | --- | --- | --- | --- |
| factual_lookup | 8 | 23/24 | 95.8% [79.8%, 99.3%] | 7/8 |
| set_retrieval | 6 | 18/18 | 100.0% [82.4%, 100.0%] | 6/6 |
| email_draft | 5 | 12/15 | 80.0% [54.8%, 93.0%] | 4/5 |
| abstention | 5 | 13/15 | 86.7% [62.1%, 96.3%] | 4/5 |

## Failed cases in detail

| Case | Origin | Runs failed | Mode | Grader reasons |
| --- | --- | --- | --- | --- |
| L02 | ground_truth | 1/3 | Tool omission | `required-tools`: required-tools: 1/2 requirement(s) unmet — carrier_history called but never with args ⊇ {"mc_number":"345678"} (args seen: {"mc_number":"345878"}) |
| D04 | ground_truth | 3/3 | Tool omission | `required-tools`: required-tools: 1/3 requirement(s) unmet — carrier_history called but never with args ⊇ {"mc_number":"712843"} (args seen: {"company_name":"Blue Ridge Transport"}) |
| A04 | ground_truth | 2/3 | Tool omission | `required-tools`: required-tools: 1/1 requirement(s) unmet — search_inquiries was never called (calls seen: carrier_history, carrier_history) |
...
| --- | --- | --- | --- | --- | --- |
| **Tool omission** | 6 | A04, D04, L02 | medium | Prompt reinforcement on the compliance gate; consider a required-tool precondition in the route. | Diagnostic, not a headline metric. |

Failure-mode fix effects measured against the baseline are recorded in the
before/after table in the final summary.

Modes with zero instances in the graded runs: Equipment blind, Lane-join blind, ASR-name echo, Phantom total, Verdict flip, Unreachable carrier, Ungrounded citation, Invented commitment, Compliance miss, Abstention break.
...
| --- | --- | --- | --- |
| L02 | 0 | **GENERATION** | gold records were retrieved and contain the answer; the answer is still wrong |
...
| --- | --- | --- | --- |
| L02 | factual_lookup | FAIL / PASS / PASS | required-tools |
| A04 | abstention | PASS / FAIL / FAIL | required-tools |
...
| --- | --- | --- | --- |
| end-to-end (ms) | 16628 | 37898 | 53291 |

**Per-tool execution time (ms)** — DB time only, excludes model latency:

| Tool | calls | median | p95 | max |
| --- | --- | --- | --- | --- |
| carrier_history | 87 | 4 | 45 | 45 |
| draft_email | 18 | 3 | 21 | 21 |
| get_load | 40 | 2 | 36 | 46 |
| market_rate | 9 | 2 | 7 | 7 |
| search_inquiries | 122 | 5 | 69 | 72 |

| Tokens | total | per run |
| --- | --- | --- |
| input | 1689432 | 23464 |
| output | 73001 | 1014 |
| cached input (read) | 984840 | 13678 |

Tool calls per run: mean 3.8, max 12. Steps per run: mean 3.3.

**Cost.** $10.27 for 72 graded runs — $0.1427 per query on `claude-opus-5`. Priced from the token counts each run recorded, using the `PRICING` map in `evals/compare.ts`; not an estimate from a per-call average.
