...
| label | **BASELINE (PARTIAL)** — no agent or tool fixes applied; 48/72 runs could not be executed |
| model | claude-opus-5 |
...
| --- | --- | --- | --- |
| **Run-level pass rate** | 18/24 (75.0%) | [55.1%, 88.0%] | every graded run, all k pooled — the expected outcome of one broker query |
| **pass@1** | 5/8 (62.5%) | [30.6%, 86.3%] | case passes if its first run passes (the k=1 slice) |
| **pass^3** | 5/8 (62.5%) | [30.6%, 86.3%] | case passes only if ALL k runs pass — our own addition, see note |
...
> 48 of 72 runs errored before producing an answer and are excluded from every denominator above.
> 16 cases have NO graded run at all: S01, S02, S03, S04, S05, S06, D01, D02, D03, D04, D05, A01, A02, A03, A04, A05.
...
| set_retrieval | 6 | not run | — | — |
| email_draft | 5 | not run | — | — |
| abstention | 5 | not run | — | — |
...
| --- | --- | --- | --- | --- | --- |
| **Equipment blind** | 3 | L05 | critical | Resolve `equipment` through the same `extracted_load_reference -> loads` subquery the lane filters already use, OR-ed with `extracted_equipment`. | S01, S05, L05 move to pass; set-retrieval bucket pass@1 rises by ~3 cases. |
...
Modes with zero instances in the graded runs: Lane-join blind, Phantom total, Unreachable carrier, Ungrounded citation, Invented commitment, Compliance miss, Abstention break, Tool omission.
...
| --- | --- | --- | --- | --- |
| lookup | **2** | **4** | 2<sup>2?</sup> | **8<sup>2?</sup>** |
| set | 3<sup>3?</sup> | 3<sup>3?</sup> | — | 6<sup>6?</sup> |
| rate | **2<sup>1?</sup>** | — | — | **2<sup>1?</sup>** |
| draft | 4<sup>4?</sup> | 1<sup>1?</sup> | — | 5<sup>5?</sup> |
| compliance | 1 | — | 2<sup>2?</sup> | 3<sup>2?</sup> |

**Data availability x time scope**

| data availability | none | this_week | window | total |
| --- | --- | --- | --- | --- |
| present | **13<sup>7?</sup>** | 3<sup>3?</sup> | **2** | **18<sup>10?</sup>** |
| absent | 5<sup>5?</sup> | — | 1<sup>1?</sup> | 6<sup>6?</sup> |
...
Tool calls per run: mean 2.5, max 4. Steps per run: mean 2.8.

**Cost.** $1.97 for 24 graded runs — $0.0821 per query on `claude-opus-5`. Priced from the token counts each run recorded, using the `PRICING` map in `evals/compare.ts`; not an estimate from a per-call average.
...
Measured versions: v1. Any prompt version shipped in
...
### Repeat-stability — PENDING (blocked on API budget)
